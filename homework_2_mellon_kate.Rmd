...
1.) The points are not blue because a manual color specification needs to be given outside of the aes mapping as an argument of the geom function.
2.) Continuous variables include displ, year, cycl, cty, and hwy.
Categorical variables include manufacturer, model, trans, drv,fl, and class.
3.) Color and size can be mapped continuously, while shape cannot be mapped to a continuous variable.
4.) Mapping the same variable to multiple aesthetics will show variation in the spectrum of values along multiple dimensions (i.e. color and size). 
5.) The stroke dimension affects the thickness of the border for certain shapes.
6.) When you map an aesthetic to something other than a variable name, then the aesthetic is mapped based on the evaluation of the argument given. In the example, the argument displ<5 is a boolean statement, so the variable color is mapped based on whether the statement evaluates to true or false.
...
2.) geom_col uses stat_identity as its default stat, while geom_bar uses stat_count as its default stat. geom_col shows the height of bars as raw values of the data, while using geom_bar utilizes the count of rows to determine the bar height.
