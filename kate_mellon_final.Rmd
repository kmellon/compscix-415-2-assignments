...
```{r}
all_means %>% summarize(se_age = sd(all_means))
...
plot_roc <- function(tbl) {
p <- ggplot(data = tbl, aes(x = fpr, y = tpr)) +
...
The random forest model performs better as it appears to have a higher auc on the ROC plot than the decision tree model.
